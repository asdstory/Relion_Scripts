...
def getNumberParticlesRelion(star):
    ...
    f1.close()
    print "total num of particles in star file particle session = %i" %(tot)
    return tot
...
#Now, loop over all particles, and check if given particle is in the 'bad list', if so, do not write to output file .
	...
	if debug is True:
		print "Number of bad particles in the area that need to be reweighted is: %i" %(badcounter)	
	...
	if debug is True:
		print "Number of lines in tmpfile is numLines=%i" (str(numLinesTemptFile))
	...
	if debug is True:
		print "numpy list of random numbers is: " (toberemoved)	
